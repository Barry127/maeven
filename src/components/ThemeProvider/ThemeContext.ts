...
import {
  CompleteTheme,
  extendFromBaseTheme
} from '../../lib/theme/extendFromBaseTheme';
import maeven from '../../themes/maeven';

export const ThemeContext = createContext<CompleteTheme>(
  extendFromBaseTheme(maeven)
);
...
body {
  ...
  transition: background var(--mvn-transition-duration)
...
::selection {
  background: var(--mvn-color-text-selection);
}`;
